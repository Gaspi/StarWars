...
let (boom_img1, boom_img2, boom_img3) = begin
	open_graph "1x1";
	let res = (
...
type joueur = {vit:float; def:float; atk:float; vol:float;
	terrifiant:bool; defenseur:bool; furtif:bool; dipl_vol:bool;
	dipl_atk:bool;dipl_def:bool; civi:culture; conquerant:bool};;
type etoile = {x:int; y:int; r:int; prod:int; num:int;
	mutable control:int; mutable pop:int};;
type vaisseau = {camp:int; mutable sens:int;
	 mutable avancee:float; mutable charge : int};;
type arete = {depart:int; arrivee:int; long:float; dist:float;
...
if force <= 0. then p else begin
	...
	let coef = force *. (p.vit ** (-.1./.3.2)) /. norme *. srd p.defenseur 0.87 *. srd p.terrifiant 0.94 *.
		srd p.dipl_def 0.95 *. srd p.dipl_vol 0.92 *. srd p.dipl_atk 0.95 *.
		srd p.furtif 0.96 *. srd p.conquerant 0.95 *. match p.civi with
		|Neanderthal -> 4.
		|Adaptee -> 1.
		|Riche -> 0.80
		|Industriel -> 0.85
...
let univers = 
	let res = Array.make taille
		{x=0;y=0;r=0;prod=0;num=0;control=0;pop=0} in
	for i = 0 to taille - 1 do
...
let get_coord ar av =
 ...
 , univers.(d).y + int_of_float (c *. float_of_int (univers.(a).y - univers.(d).y)) in
...
let lance_vaisseau camp charge depart arrivee =
	...
	|Indirect ar ->
		ar.trafic <- {camp=camp;sens=(-1);avancee=ar.dist;charge=charge}::ar.trafic in
...
if univers.(i).control = n then begin
	...
	for jp = 0 to taille - 1 do
		...
		match tab_aretes.(i).(jp) with
		 ...
		 |Direct ar| Indirect ar -> begin
			let score = (foi u.pop +. (foi vr +. 0.2) *. ar.dist /. (4. *. temps_paye)) *.
			...
			let score2 = score -. foi
				(if drev >=0 then (if score < 0. then 50*vrai_revenu u.prod j.(n).civi else 0)
	...
	if !best_target >= 0 then (
		...
		univers.(i).pop <- 0)
	else if not !frontiere then begin
		let rep = ref (-1) in
...
let rec cherche_bagarre v inf sup ar = function
	...
	|v2::q -> if v.camp <> v2.camp && v2.avancee >= inf && v2.avancee <= sup then begin
		...
		cherche_bagarre v inf sup ar q in
...
let actu_univers () =
	...
	|t::q ->
		...
		aux planete q in
	for i = 0 to taille - 1 do
		aux univers.(i) arrivees.(i) done in
...
let fond = begin
	...
	synchronize ();
get_image 0 0 taillex tailley end in
...
let aff_univers s =
	...
	set_color white;
	for i = 0 to 2 do
	if s.(i) <> (-1) then (let p = univers.(s.(i)) in
		draw_circle p.x p.y p.r;
		draw_circle p.x p.y (p.r+1);
		draw_circle p.x p.y (p.r-1)) done;
	synchronize () in
...
let traite_touche = function
	...
	|c -> match element c touches1 with
		|None -> if not ordi.(c2) then begin
			...
			|Some i ->
				if select.(1) = (-1) then
					(if univers.(i).control = c2 + 1 then select.(1) <- i)
				else begin
...
while !continue do
	...
	aff_univers select;
	if Sys.time () -. !chrono_paye > temps_paye *. !vitesse then (jour_de_paye ();
		chrono_paye := !chrono_paye +. temps_paye *. !vitesse);
	if Sys.time () -. !chrono_actu > !vitesse then (actu_univers ();
