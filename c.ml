...
let string_of_int_list l =
	...
	let rec aux i = function
		|[] -> ()
		|t::q -> s.[i] <- char_of_int t; aux (i+1) q in
	aux 0 l; s
...
let string_of_char_list l =
	...
	let rec aux i = function
		|[] -> ()
		|t::q -> s.[i] <- t; aux (i+1) q in
	aux 0 l; s

let is_prime n =
	let i = ref 2 in
	while !i * !i <= n && n mod !i <> 0 do incr i done;
	!i * !i > n
...
let coder ?(code=97) ?(decalage=147) ?(graine=(0,0,0)) message =
	...
	let res = String.create (2*t+3) in
...
let decoder ?(code=97) ?(decalage=147) message =
	...
	let res = String.create ((t - 3) / 2) in
