...
let neutre_base = {vit = 1.; def=1.1; atk=1.; vol=1.; terrifiant=false;
	defenseur = false; furtif=false; dipl_vol=false; dipl_atk = false;
	dipl_def=false;conquerant=false;civi=Neanderthal};;

let ordi_base = {vit = 1.1; def=1.1; atk=1.; vol=1.; terrifiant=false;
...
let presentation_perso s =
	presentation_pers (perso_of_string s);;

let creer_perso ?(conquerant=false) ?(furtif=false) ?(diplomatie_vol=false)
	 ?(diplomatie_defense=false) ?(diplomatie_attaque =false) ?(defenseur=false) ?(terrifiant=false)
	?(civilisation=Adaptee) ~attaque ~defense ~combat_spatial ~vitesse nom =
	let a = equilibrer 1. 	{atk=attaque; def=defense; vol=combat_spatial; vit=vitesse; conquerant=conquerant;
		furtif=furtif; dipl_vol=diplomatie_vol; dipl_def =diplomatie_defense; dipl_atk=diplomatie_attaque;
		defenseur=defenseur; terrifiant=terrifiant; civi=civilisation } in
